...
export const ProStudentHub = () => {
  ...
  const isPro = user?.subscription?.plan === 'pro' || user?.subscription?.plan === 'premium' || user?.subscription?.plan === 'business';
  ...
  const handleUpgradeToPro = async () => {
    setUpgrading(true);
    try {
      const res = await api.post('/pro/student/upgrade', { plan: 'pro' });
      if (user) {
        setUser({ ...user, subscription: { ...user.subscription, plan: 'pro' } });
      }
      toast.success('⭐ Congratulations! Pro Student Workspace Unlocked!');
    } catch (err) {
  ...
  return (
    <div className="page container animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '28px', paddingBottom: '60px' }}>
      ...
      <div className="card glass" style={{
        ...
        flexWrap: 'wrap',
        gap: '20px',
        background: 'linear-gradient(135deg, rgba(225,29,72,0.08) 0%, var(--bg-surface) 100%)',
        border: '1px solid var(--red-border)',
        boxShadow: '0 0 32px var(--red-subtle)'
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div className="section-label" style={{ marginBottom: 0 }}>TuniStudy Pro Workspace</div>
              <span style={{
                ...
                {isPro ? '● PRO PLAN ACTIVE' : '○ FREE PLAN (TRIAL MODE)'}
              </span>
            </div>
        ...
        {!isPro && (
          <button 
            ...
            style={{ padding: '12px 24px', fontSize: '0.95rem', boxShadow: '0 0 20px var(--red-glow)' }}
          >
            {upgrading ? '⏳ Activating Pro...' : '⚡ Unlock Full Pro Pass'}
          </button>
        )}
      </div>
      ...
      <div style={{ display: 'flex', gap: '10px', borderBottom: '1px solid var(--glass-border)', paddingBottom: '10px' }}>
        {[
          { id: 'tasks', label: `📋 Pro Task Manager (${tasks.length})`, icon: '📋' },
          { id: 'calculator', label: `🧮 Score & GPA Calculator`, icon: '🧮' },
          { id: 'advisor', label: `🤖 AI Career & University Advisor`, icon: '🤖' },
        ].map(tab => (
          <button
            key={tab.id}
      ...
      {activeTab === 'tasks' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
            <div>
              <h3>Academic Milestone & Task Board</h3>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Track exams, revision schedules, internship deadlines, and university applications.</p>
            </div>
              ...
              return (
                <div key={col.status} className="card glass" style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '12px', minHeight: '320px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--glass-border)', paddingBottom: '10px' }}>
                    <span style={{ fontWeight: 700, color: col.color, fontSize: '0.92rem' }}>{col.label}</span>
                        ...
                        >
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            <span style={{
                              ...
                              background: t.priority === 'urgent' ? 'rgba(225,29,72,0.15)' : t.priority === 'high' ? 'rgba(251,191,36,0.15)' : 'var(--bg-elevated)',
                              color: t.priority === 'urgent' ? 'var(--red)' : t.priority === 'high' ? '#fbbf24' : 'var(--text-secondary)',
                            }}>
                              {t.priority}
                            </span>
                            <button onClick={() => handleDeleteTask(t._id)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '0.9rem' }}>
                              🗑️
                            </button>
                          </div>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '4px', paddingTop: '8px', borderTop: '1px solid var(--glass-border)' }}>
                            <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>
                              {t.dueDate ? `📅 Due: ${new Date(t.dueDate).toLocaleDateString()}` : `🏷️ ${t.category}`}
                            </span>
                            <button 
                ...
                <form onSubmit={handleCreateTask} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <div className="form-group">
                    ...
                    <textarea 
                      ...
                      placeholder="Add sub-items, reference links, or notes..."
                    />
                  </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: calcResult ? '1.2fr 1fr' : '1fr', gap: '24px' }}>
              ...
              {calcResult && (
                <div className="card glass animate-scale-in" style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: '16px', height: 'fit-content' }}>
                  <div className="section-label">Orientation Result</div>
                  <div>
                    <div style={{ fontSize: '3rem', fontWeight: 800, color: 'var(--red)', fontFamily: 'var(--font-display)', lineHeight: 1 }}>
                  ...
                  <div style={{ borderTop: '1px solid var(--glass-border)', paddingTop: '12px' }}>
                    ...
                    <ul style={{ paddingLeft: '18px', fontSize: '0.84rem', display: 'flex', flexDirection: 'column', gap: '6px', color: 'var(--text-secondary)' }}>
                      {calcResult.formuleGlobale >= 160 ? (
                        <>
                          <li>🏛️ <strong>INSAT (MPI / CBA)</strong> — Highly Eligible</li>
                          <li>🎓 <strong>TBS (Tunis Business School)</strong> — Eligible</li>
                          <li>🏛️ <strong>Preparatory Classes (IPEST, EPAM, IPEIT)</strong></li>
                        </>
                      ) : calcResult.formuleGlobale >= 135 ? (
                        <>
                          <li>🎓 <strong>FST Tunis</strong> — Computer Science / Data</li>
                          <li>🏛️ <strong>ISG / IHEC Tunis</strong> — Business & Finance</li>
          ...
          {calcType === 'university_gpa' && (
            <div style={{ display: 'grid', gridTemplateColumns: calcResult ? '1.3fr 1fr' : '1fr', gap: '24px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  <div>
                    ...
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', margin: '4px 0 0' }}>
                      Calculate weighted average (Moyenne Générale) across semester modules and ECTS credits.
                    </p>
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {uniModules.map((m, idx) => (
                    <div key={idx} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr auto', gap: '10px', alignItems: 'center', padding: '10px', background: 'var(--bg-elevated)', borderRadius: 'var(--r-md)' }}>
                      ...
                      <button onClick={() => handleRemoveModule(idx)} style={{ background: 'none', border: 'none', color: 'var(--red)', cursor: 'pointer', padding: '4px' }}>
                        ✕
                      </button>
                    </div>
                  ))}
              ...
              {calcResult && (
                <div className="card glass animate-scale-in" style={{ padding: '28px', display: 'flex', flexDirection: 'column', gap: '16px', height: 'fit-content' }}>
                  <div className="section-label">Academic Standing</div>
                  <div>
                    <div style={{ fontSize: '3rem', fontWeight: 800, color: calcResult.validated ? '#34d399' : 'var(--red)', fontFamily: 'var(--font-display)', lineHeight: 1 }}>
                  ...
                  <div style={{ padding: '14px', background: calcResult.validated ? 'rgba(52, 211, 153, 0.1)' : 'rgba(225, 29, 72, 0.1)', border: `1px solid ${calcResult.validated ? '#34d399' : 'var(--red)'}`, borderRadius: 'var(--r-md)' }}>
                    <div style={{ fontWeight: 700, color: calcResult.validated ? '#34d399' : 'var(--red-bright)' }}>
                      {calcResult.validated ? '🎉 Semester Validated (Admis)' : '⚠️ Semester Incomplete (Ajourné / Rachat)'}
                    </div>
            ...
            {aiChat.map((msg, idx) => (
              ...
              >
                <div style={{
                  ...
                  background: msg.sender === 'user' ? 'var(--red)' : 'var(--bg-elevated)',
