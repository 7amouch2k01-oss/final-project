...
const seed = async (clearExisting = true) => {
  try {
    ...
    if (require.main === module && process.env.WRITE_README !== 'true') {
...
const writeReadme = () => {
  const fs = require('fs');
  const readmeContent = `# 🎓 TuniStudy & TuniJob — Next-Gen Tunisian Career & Academic Super-Platform
...
`;
  ...
  console.log('✅ README.md generated successfully with portfolio link: https://mosma.vercel.app');
};

if (process.env.WRITE_README === 'true') {
  writeReadme();
}
